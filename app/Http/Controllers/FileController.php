...
class FileController extends Controller
{
    ...
    //         // Your data for the page (if needed)
    //     ]);
    // }
    ...
    public function getStorageUsage()
        {
            $userId = Auth::id();
            // Assuming each file record has a 'size' column storing file size in bytes
            $usedStorage = File::where('created_by', $userId)->sum('size');

            // Convert bytes to GB, adjust as necessary
            $usedStorageGB = number_format($usedStorage / (1024 * 1024 * 1024), 2);

            // Total storage in GB (this can be a user-specific value if needed)
            $totalStorageGB = 32;
